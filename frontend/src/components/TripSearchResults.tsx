...
import {
  ...
  Sparkles, ChevronRight, X, ExternalLink,
...
export function TripSearchResults({ tripId, destination, onSavedToBudget }: Props) {
  ...
  const [moreSheet, setMoreSheet] = useState<{ open: boolean; kind: "flight" | "hotel" | null; item: any }>(
  ...
  return (
    <View style={{ gap: spacing.md }}>
      ...
      {(data.flights || []).map((f: any, i: number) => (
        <FlightCard
          key={`f${i}`} flight={f} onOpen={openUrl}
          onMore={() => setMoreSheet({ open: true, kind: "flight", item: f })}
        />
      ))}
      ...
      {(data.hotels || []).map((h: any, i: number) => (
        <HotelCard
          ...
          phMode={!!extras.philippines_mode}
          onMore={() => setMoreSheet({ open: true, kind: "hotel", item: h })}
        />
      ))}
      ...
      <MoreSheet
        state={moreSheet}
        onClose={() => setMoreSheet({ open: false, kind: null, item: null })}
        onOpen={openUrl}
      />
    </View>
  );
...
function FlightCard({ flight, onOpen, onMore }: { flight: any; onOpen: (u?: string) => void; onMore: () => void }) {
    ...
    <View style={styles.card}>
      ...
      <View style={{ padding: spacing.md, gap: 6 }}>
        ...
        <View style={styles.btnRow}>
          <TouchableOpacity style={[styles.btnPrimary, { backgroundColor: color }]} onPress={() => onOpen(flight.booking_url_airline)}>
            <Text style={styles.btnPrimaryText} numberOfLines={1}>Book on {flight.airline}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.btnOutline} onPress={() => onOpen(flight.google_flights)}>
            <Text style={styles.btnOutlineText}>Google Flights</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.btnOutline} onPress={() => onOpen(flight.kayak)}>
            <Text style={styles.btnOutlineText}>Kayak</Text>
          </TouchableOpacity>
        </View>
        <TouchableOpacity onPress={onMore} style={{ alignSelf: "center", marginTop: 4 }}>
          <Text style={styles.moreLink}>More options ›</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
function HotelCard({ hotel, onOpen, phMode, onMore }: { hotel: any; onOpen: (u?: string) => void; phMode: boolean; onMore: () => void }) {
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={{ padding: spacing.md, gap: 6 }}>
        ...
        <View style={styles.hotelBtnGrid}>
          <TouchableOpacity style={[styles.btnPrimary, { backgroundColor: color }]} onPress={() => onOpen(hotel.booking_url_hotel_direct)}>
            <Text style={styles.btnPrimaryText}>Book Direct</Text>
          </TouchableOpacity>
          {phMode ? (
            <TouchableOpacity style={[styles.btnOutline, { borderColor: "#FF9500" }]} onPress={() => onOpen(hotel.booking_url_agoda)}>
              <Text style={[styles.btnOutlineText, { color: "#FF9500" }]}>Agoda · Best for Asia</Text>
            </TouchableOpacity>
          ) : null}
          <TouchableOpacity style={styles.btnOutline} onPress={() => onOpen(hotel.booking_url_booking_com)}>
            <Text style={styles.btnOutlineText}>Booking.com</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.btnOutline} onPress={() => onOpen(hotel.booking_url_hotels_com)}>
            <Text style={styles.btnOutlineText}>Hotels.com</Text>
          </TouchableOpacity>
          {!phMode ? (
            <TouchableOpacity style={[styles.btnOutline, { borderColor: "#FF9500" }]} onPress={() => onOpen(hotel.booking_url_agoda)}>
              <Text style={[styles.btnOutlineText, { color: "#FF9500" }]}>Agoda</Text>
            </TouchableOpacity>
          ) : null}
        </View>
        <TouchableOpacity onPress={onMore} style={{ alignSelf: "center", marginTop: 4 }}>
...
function MoreSheet({ state, onClose, onOpen }: {
  state: { open: boolean; kind: "flight" | "hotel" | null; item: any };
  onClose: () => void; onOpen: (u?: string) => void;
}) {
  if (!state.open || !state.item) return null;
  const item = state.item;
  const options: { label: string; url?: string }[] = state.kind === "flight" ? [
    { label: "Expedia", url: item.expedia },
    { label: "Priceline", url: item.priceline },
    { label: "Skyscanner", url: item.skyscanner },
    { label: "Momondo", url: item.momondo },
    { label: `Airline direct: ${item.airline}`, url: item.booking_url_airline },
  ] : [
    { label: "Booking.com", url: item.booking_url_booking_com },
    { label: "Hotels.com", url: item.booking_url_hotels_com },
    { label: "Agoda", url: item.booking_url_agoda },
    { label: "Expedia", url: item.booking_url_expedia },
    { label: "Priceline", url: item.booking_url_priceline },
    { label: "Kayak", url: item.booking_url_kayak },
    { label: `Hotel direct: ${item.hotel_name}`, url: item.booking_url_hotel_direct },
  ];
  return (
        ...
        <Pressable style={{ width: "100%" }} onPress={(e) => e.stopPropagation()}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHead}>
              <Text style={styles.modalTitle}>All booking options</Text>
              <TouchableOpacity onPress={onClose}><X size={18} color={colors.textSecondary} /></TouchableOpacity>
            </View>
            <ScrollView style={{ maxHeight: 400 }}>
              {options.filter((o) => o.url).map((o, i) => (
                <TouchableOpacity key={i} style={styles.modalRow} onPress={() => { onOpen(o.url); onClose(); }}>
...
const styles = StyleSheet.create({
  ...
  modalBackdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.55)", justifyContent: "flex-end", padding: spacing.md },
  modalSheet: {
    backgroundColor: colors.surface, borderRadius: radius.md, padding: spacing.md, gap: 4,
  ...
  modalRowText: { color: colors.textPrimary, fontSize: 12, fontWeight: "600" },
});
