// Shared Resume Vault panel — used by both /resume-hub (in a tab) and
// as a standalone reference. Owns its own state (list, upload, paste, delete,
// set-default) so it can drop into any parent screen.
...
import {
  Upload, Trash2, Star, StarOff, FileText, ClipboardPaste, CheckCircle2,
...
export type ResumeVaultPanelHandle = { refresh: () => Promise<void> };
  ...
  return (
    <View style={{ gap: spacing.md }}>
      ...
      {loading ? (
        ...
        <ScrollView scrollEnabled={false} contentContainerStyle={{ gap: spacing.sm }}>
          {items.map((item) => {
            const meta = TYPE_META[item.file_type];
            const busyThis = busy === item.resume_id;
            return (
              <View key={item.resume_id} style={styles.card} testID={`resume-${item.resume_id}`}>
